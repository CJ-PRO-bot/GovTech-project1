// Frontend wired to backend API (Express + SQLite)
(function(){
  ...
  formSignup.addEventListener('submit', async (e) => {
    ...
    setLoading(btn, true); await sleep(400);
    const payload = {
  ...
  async function ensureUserAndRender(){
    const me = await api.userMe().catch(()=>null);
    if(me && me.user){ renderDashboard(me.user); }
  }

  function init(){
    footerYear.textContent = new Date().getFullYear();
    api.me().then(async (m)=>{
      if(m && m.user){ const u = await api.userMe(); renderDashboard(u.user); }
      else { showLogin(); }
    });
    tick();
  }
